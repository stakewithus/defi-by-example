...
#   def swapExactTokensForTokens(amountIn: uint256, amountOutMin: uint256, path: address[3], to: address, deadline: uint256): nonpayable

UNISWAP_V2_ROUTER: constant(address) = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
...
def swap(tokenIn: address, tokenOut: address, amountIn: uint256):
  ...
  # path: address[3] = [ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS]
  # if tokenIn == WETH or tokenOut == WETH:
  #   path[0] = tokenIn
  #   path[1] = tokenOut
  # else:
  #   path[0] = tokenIn
  ...
  # UniswapV2Router(UNISWAP_V2_ROUTER).swapExactTokensForTokens(amount, 0, path, self, block.timestamp + 1800)
